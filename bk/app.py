        ...
        if st.session_state["run_mode"] == "api":
            ...
            with col1:
                temperature = st.slider(
                    "Temperature", 0.0, 1.0, cfg.temperature, 0.1,
                    help="越低越严谨，越高越创造性",
                )
            with col2:
...
def render_step1_material():
    ...
    with col_right:
        ...
        if uploaded_files:
            ...
            st.success("✅ 新材料已 OCR 完成并加入知识库！")
            st.session_state["uploaded_new"] = True
            safe_rerun()

        st.divider()
        if st.button("📥 批量 OCR: input/ → output_md", use_container_width=True,
                     help="扫描 input/ 目录下所有 PDF，批量 OCR 输出到 output_md/", type="secondary"):
            pdf_list = sorted(cfg.input_dir.glob("*.pdf"))
            if not pdf_list:
                st.warning("input/ 目录下没有 PDF 文件")
            else:
                batch_bar = st.progress(0.0, text=f"正在 OCR 共 {len(pdf_list)} 个 PDF...")
                batch_status = st.empty()
                results = []
                for idx, pdf_path in enumerate(pdf_list):
                    batch_status.caption(f"处理中: [{idx+1}/{len(pdf_list)}] {pdf_path.name}")
                    try:
                        md_path = process_pdf_to_md(pdf_path, cfg.output_md_dir)
                        results.append(md_path)
                        add_log(f"OCR 完成: {pdf_path.name} -> {md_path.name}")
                    except Exception as e:
                        st.error(f"OCR 失败: {pdf_path.name} - {e}")
                        add_log(f"OCR 失败: {pdf_path.name} - {e}")
                    batch_bar.progress((idx + 1) / len(pdf_list))
                batch_status.empty()
                if results:
                    batch_bar.progress(1.0, text=f"✅ {len(results)} 个 PDF 完成")
                    add_log(f"批量 OCR 完成: {len(results)} 个 PDF -> {cfg.output_md_dir}")
                    st.success(f"批量 OCR 完成，共生成 {len(results)} 个文件！")
                    st.session_state["uploaded_new"] = True
                    safe_rerun()
...
def render_step3_download():
    ...
    with tabs[2]:
        ...
        if docx_path and Path(docx_path).exists():
            ...
            st.caption("暂无 Word 文档，请在学习资料 Tab 中点击「转为 Word」")
